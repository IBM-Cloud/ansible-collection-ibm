...
class Terraform:
    ...
    def _download_extract_zip(self, url):
        ...
        zip_archive.extractall(path=self.directory)

    def _install_terraform(self):
        self._download_extract_zip(
            "{0}{1}/terraform_{1}_linux_amd64.zip".format(
                self.TERRAFORM_BASE_URL, self.terraform_version))
        filename = 'terraform'
        os.chmod(os.path.join(self.directory, filename), 0o755)
        os.replace(
            os.path.join(self.directory, filename),
            os.path.join(self.terraform_dir, filename))
        self.executable = os.path.join(
            self.terraform_dir, 'terraform')

    def _install_ibmcloud_tf_provider(self):
        self._download_extract_zip(
            "{0}v{1}/linux_amd64.zip".format(
                self.IBM_PROVIDER_BASE_URL, self.ibm_provider_version))
